...
| 图 | 口径 | 为什么 |
|---|---|---|
| 托管账户 | **本金口径**：总资产 ÷ 当日净入金 − 1 | 跟 Robinhood App 上那个数一致，一眼能对上 |
| 毛毛的资产 | **时间加权**：逐日复利链 | 她的钱分几次进来，只能按当日净值买份额 |

托管账户曾经也用时间加权，结果和 App 上差了 4 个百分点（2.31% vs 6.55%）：
账户快照是隔日的，一笔入金落在空档日就被当成当天的暴涨，链式相乘把误差滚大了。
本金口径没有这个毛病，代价是它不区分「什么时候赚的」—— 但账户这条线本来也
不需要发份额。

净入金的起点是 `src/data/broker-adjustments.json` 的 `initialPrincipal`，
往后按 `CONTRIBUTION` / `WITHDRAWAL` / `FEE` 逐日累加。
